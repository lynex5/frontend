...
export default function About() {
  ...
  useEffect(() => {
    Promise.all([get('/about'), get('/skills'), get('/stats'), get('/timeline')])
      .then(([aboutData, skillsData, statsData, timelineData]) => {
        if(aboutData && aboutData.length > 0) setAbout(aboutData[0]);
  ...
  const handleDownload = async () => {
    try {
      ...
      if (response.ok) {
        ...
        navigate('/error', { 
          state: { 
            ...
            message: 'RESUME NOT FOUND_', 
            detail: 'The requested resume could not be located on the server. It may have been deleted or not uploaded yet.'
          } 
        });
      }
    } catch (e) {
      ...
      navigate('/error', { 
        state: { 
          code: '500', 
          message: 'CONNECTION ERROR_', 
          detail: 'Failed to establish a connection to the file server.'
        } 
      });
    }
  };
